...
@section('content')
            ...
            <div class="card shadow">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">
                        <i class="fas fa-edit me-2"></i>Modifier la Facture #{{ $facture->num_facture }}
                    </h6>
                </div>
                <div class="card-body">
                    <form action="{{ route('factures.update', $facture->id) }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                <label for="num_facture" class="form-label">N° Facture <span
                                        class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('num_facture') is-invalid @enderror"
                                    id="num_facture" name="num_facture"
                                    value="{{ old('num_facture', $facture->num_facture) }}" required>
                                @error('num_facture')
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="date" class="form-control @error('date') is-invalid @enderror" <input
                                    type="date" id="date" name="date"
                                    value="{{ old('date', \Carbon\Carbon::parse($facture->date)->format('Y-m-d')) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <div id="produits-container">
                                @foreach (old('produits', $facture->products) as $index => $produit)
                                    <div class="row produit-item mb-3">
                                        <div class="col-md-6">
                                            <select class="form-select produit-select"
                                                name="produits[{{ $index }}][id]" required>
                                                <option value="">Sélectionner un produit</option>
                                                @foreach ($produits as $p)
                                                    <option value="{{ $p->id }}"
                                                        {{ (is_array($produit) ? $produit['id'] : $produit->id) == $p->id ? 'selected' : '' }}>
                                                        {{ $p->nom }} ({{ $p->prix }} €)
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-md-4">
                                            <input type="number" class="form-control"
                                                name="produits[{{ $index }}][qte]" placeholder="Quantité"
                                                min="1"
                                                value="{{ is_array($produit) ? $produit['qte'] : $produit->pivot->qte }}"
                                                required>
                                        </div>
                                        <div class="col-md-2">
                                            @if ($loop->first)
    ...
    <script>
        ...
        `;
            ...
            // Supprimer un produit (sauf le premier)
            document.addEventListener('click', function(e) {
                if (e.target.classList.contains('remove-produit')) {
                    if (document.querySelectorAll('.produit-item').length > 1) {
                        ...
                        document.querySelectorAll('.produit-item').forEach((item, index) => {
                            item.querySelector('[name^="produits["]').name =
                                `produits[${index}][id]`;
                            item.querySelector('[type="number"]').name = `produits[${index}][qte]`;
                        });
                    }
                }
            });
